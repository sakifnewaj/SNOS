...
async function registerApp(appId, capabilities) {
	...
	if (!initialization)
		notify(await getFileNameByID(appId) + " installed", "Registered " + capabilities.toString(), "SNOS System");
...
async function initializeOS() {
	...
	await say(`
		...
		<p>By using Nova OS, you agree to the <a href="SNOS">Terms of Use</a>. Which mentions the intended purpose of this system.
	...
	setdb().then(async function () {
		...
		await ensureAllSettingsFilesExist()
			...
			.then(async () => {
				...
				notify("Welcome to SNOS, " + CurrentUsername + "!", "I really think you'd enjoy it!", "SNOS")
...
document.addEventListener("DOMContentLoaded", async function () {
	...
	async function startfunctions() {
		...
		onstartup.push(async () => {
			...
			if (await justConfirm("New version available."," Support for the current version (SNOS adthoughtsglobal) will end in December 2025. You can still use this version until end of support by clicking NO. Click YES for changing the version. ", "success"))
			location.replace("https://sakifnewaj.github.io/SNOS/");
		});
