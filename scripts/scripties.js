...
async function checkAndRunFromURL() {
	...
	if (run === 'erdbsfull') {
		let x = await justConfirm("Reset all your data?", "The link you opened SNOS had a param to erase your device. Do this only if its instructed to do so by SNOS developers.");
		if (x) {
			erdbsfull();
		}
	}
	...
	if (filePath) {
		console.log(`Opening SNOS path: ${filePath}`);
...
async function useNovaOffline() {
	if (await justConfirm("Turn on offline mode?", "Offline mode saves a copy of SNOS (~5MB) in your browser.")) {
